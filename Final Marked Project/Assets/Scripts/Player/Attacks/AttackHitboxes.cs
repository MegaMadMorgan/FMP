...
public class AttackHitboxes : MonoBehaviour
{
    ...
    // alot of code is going to be duplicated here, i will not annotate all of them they are the same thing again and again...

    #region Throw The Item
    public void Throw()
    {
        // check what item the player is using
    ...
    public void BBA1()
    {
        // when this is called in the animation, check if player is attacking
        if (transform.GetComponentInParent<PlayerMovement>().AttackTime > 0.01)
        {
            // get player rotation
            Quaternion playerRotation = this.transform.rotation;

            // and create hitbox accordingly
    ...
    public void MA11()
    {
        if (PlayerAnimator.GetInteger("Anim") == 32)
        {
            ...
            Vector3 playerPos = GameObject.Find("Third-Person Player").transform.position;
            Vector3 playerDirection = GameObject.Find("Third-Person Player").transform.forward;
            // make the position of the hitbox spawn
            Vector3 HBPos = playerPos + (playerDirection * 0.8f);
            // make hitbox
            ...
            {
                // if so keep player in air by giving a little bounce
                Rigidbody rb = transform.parent.GetComponent<Rigidbody>();
                rb.AddForce(-rb.velocity, ForceMode.VelocityChange);
                rb.AddForce(0, 5, 0, ForceMode.Impulse);
                // add to the air time count down
    ...
    {
        // check if in uppercut animation
